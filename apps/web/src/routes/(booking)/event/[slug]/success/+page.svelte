<script lang="ts">
  ...
  const formattedDate = $derived(start ? new Date(start).toLocaleString("en", { weekday: "long", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "");

  async function cancelBooking() {
    if (!confirm("Are you sure you want to cancel this booking?")) return;
    const res = await fetch(`/api/v1/calendar/public/bookings/${cancelToken}/cancel`, { method: "POST" });
...
<div class="min-h-screen bg-gray-50 flex items-center justify-center px-4">
  <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-10 max-w-md w-full text-center">
    ...
    {#if formattedDate}
      <div class="bg-gray-50 rounded-xl p-4 mb-6 text-sm text-gray-700 font-medium">{formattedDate}</div>
    {/if}
    {#if cancelToken}
      <button onclick={cancelBooking} class="text-sm text-gray-400 hover:text-red-500 transition-colors">Cancel this booking</button>
    {/if}
  </div>
</div>
