...
<div class="p-6">
  ...
  <div class="grid grid-cols-3 gap-4 mb-8">
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      ...
      <p class="text-2xl font-bold text-gray-900">{data.metrics.totalResponses}</p>
    </div>
    <div class="rounded-lg border border-gray-200 bg-white p-4">
    ...
    <div class="space-y-2">
      {#each [5, 4, 3, 2, 1] as rating}
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="w-12 text-right text-sm text-gray-500">{ratingCount}</span>
        </div>
      {/each}
    </div>
  </div>

  <h3 class="text-sm font-medium text-gray-700 mb-3">Recent Responses</h3>
  <table class="w-full">
    <thead class="bg-gray-50">
      <tr>
        <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">Conversation</th>
        <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">Rating</th>
        <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500">Feedback</th>
