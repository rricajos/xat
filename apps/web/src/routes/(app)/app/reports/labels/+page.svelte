...
<div class="p-6">
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Label Reports</h2>
    <div class="flex items-center gap-2">
      ...
      {/each}
      <a href="/api/v1/reports/export?type=labels&days={data.days}" download class="flex items-center gap-1.5 rounded-md border border-gray-300 px-3 py-1 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400 dark:hover:bg-gray-800">
  ...
  <table class="w-full">
    ...
    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
      {#each data.labels as row}
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
          <td class="px-4 py-3">
            <span
              class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium"
              style="background-color: {row.label.color}20; color: {row.label.color}"
            >
              <span class="h-2 w-2 rounded-full" style="background-color: {row.label.color}"></span>
              {row.label.title}
            </span>
          </td>
