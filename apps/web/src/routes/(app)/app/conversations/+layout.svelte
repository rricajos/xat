    ...
    <div class="flex w-80 flex-col border-r border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <div class="flex items-center justify-between border-b border-gray-100 px-3 py-2.5 dark:border-gray-800">
        <div class="flex items-center gap-2">
          ...
          <span class="rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-semibold text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">
            Open
          </span>
        </div>
        <div class="flex items-center gap-1">
          <!-- Sort -->
          <button class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-800" title="Sort">
          ...
          <!-- Filter -->
          <button class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-800" title="Filter">
            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
      ...
      <div class="flex border-b border-gray-100 dark:border-gray-800">
        {#each [
          ...
          <a
            href={tab.href}
            class="flex-1 py-2 text-center text-[11px] font-medium transition-colors {$page.url.href.includes(tab.href.split('?')[1] ?? '__none__') || (tab.label === 'All' && !$page.url.searchParams.has('status'))
      ...
      <!-- Filters -->
      <ConversationFilters customFilters={data.customFilters as { id: number; name: string; query: Record<string, unknown> }[]} inboxes={($page.data.inboxes ?? []) as { id: number; name: string; channelType: string }[]} />
