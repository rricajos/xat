<script lang="ts">
  ...
  let form = $state({
    ...
    description: "",
    duration: 30,
    locationType: "VIDEO_DYTE",
    color: "#3B82F6",
    scope: "USER",
    teamId: "",
    notificationInboxIds: [] as number[],
    bufferMinutes: 0,
    minNoticeHours: 1,
    maxBookingsPerDay: 0,
  });
  ...
  async function save(e: SubmitEvent) {
    ...
    saving = true;
    error = "";
    const res = await fetch("/api/v1/calendar/event-types", {
      method: "POST",
    ...
    if (res.ok) {
      ...
      error = body.error ?? "Failed to create event";
      saving = false;
    }
  }
</script>
...
<div class="p-6 max-w-xl">
  <div class="mb-6 flex items-center gap-3">
    ...
    <h1 class="text-xl font-semibold text-gray-900 dark:text-white">New Event</h1>
  </div>
  ...
  <form onsubmit={save} class="space-y-5">
    <div>
      ...
      <input id="ev-name" type="text" bind:value={form.name} oninput={autoSlug} required
    ...
    <div class="p-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg">
      ...
      <div class="flex items-center gap-2">
        ...
        {:else}
          <span class="flex-1 text-sm font-mono text-gray-400 truncate" title={form.scope === "USER" ? "Set your calendar username in profile settings first" : ""}>{previewUrl}</span>
        {/if}
