...
<div class="p-6">
  ...
  {#if showForm}
    <form method="POST" action="?/invite" use:enhance class="mb-6 rounded-lg border border-gray-200 p-4 space-y-4">
      <div>
    ...
    {#each data.agents as agent}
      <div class="flex items-center justify-between rounded-lg border border-gray-200 px-4 py-3">
        <div class="flex items-center gap-3">
          <div class="relative">
            <div class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-xs font-medium text-gray-600">
              {agent.name.slice(0, 2).toUpperCase()}
            </div>
            <span
              class="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white {availabilityLabels[agent.availability ?? 2]?.color}"
            ></span>
          </div>
          <div>
