...
<div class="p-6">
  ...
  {#if showForm}
    <form method="POST" action="?/create" use:enhance class="mb-6 rounded-lg border border-gray-200 p-4 space-y-4">
      <div>
        <label for="shortCode" class="block text-sm font-medium text-gray-700">
          Short Code
        </label>
  ...
  <div class="space-y-2">
    {#each data.cannedResponses as response}
      <div class="flex items-start justify-between rounded-lg border border-gray-200 px-4 py-3">
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-gray-900">/{response.shortCode}</p>
          <p class="mt-1 text-xs text-gray-500 truncate">{response.content}</p>
        </div>
    ...
    {#if data.cannedResponses.length === 0}
      <p class="text-center text-sm text-gray-400 py-8">No canned responses yet</p>
    {/if}
  </div>
</div>
